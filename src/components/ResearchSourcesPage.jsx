...
import { RESEARCH_SOURCES } from '../constants.js';
            ...
            <div className="space-y-4">
                {RESEARCH_SOURCES.map(source => (
                    <div key={source.id} className="bg-white rounded-xl p-4 shadow-md border-l-4 border-[#367C2B]">
                        ...
                        {source.url && <a href={source.url} target="_blank" rel="noopener noreferrer" className="text-[#367C2B] font-semibold text-sm underline mt-3 inline-block">Visit Extension Website →</a>}
                    </div>
                ))}
            </div>
        </div>
